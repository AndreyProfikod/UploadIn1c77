...
Процедура ВыгрузитьПоступлениеУслуг(Команда)
	...
	ТаблицаДокументов = ПодготовитьСписокПоступлениеУслугДляВыгрузкиНаСервере();
	...
	Для каждого СтрДокумент Из ТаблицаДокументов Цикл
		...
		Если Не Отказ Тогда
			...
			Если СтруктураДанных.ПерезаписатьДокумент Тогда
				Сообщить("В 1с7 перезаписан документ №" + СтруктураДанных.НомерДок  + " от " + СтруктураДанных.ДатаДок);
			Иначе
				Сообщить("Документ №" + СтруктураДанных.НомерДок + " от " + СтруктураДанных.ДатаДок + " выгружен успешно!");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;     
...
Функция ПодготовитьСписокПоступлениеУслугДляВыгрузкиНаСервере()
	...
	МассивСсылок = Новый Массив;
	Для Каждого СтрокаДанных Из СписокПоступленийУслуг Цикл  
...
Процедура ПодготовитьПоступлениеУслугДляВыгрузки(СтруктураДанных, Отказ) Экспорт 
	...
	Попытка
		...
		Если СтруктураДанных.OLE_СпрФирмы.НайтиПоРеквизиту("ИНН", ОрганизацияИНН) <> 0 Тогда
			ФирмаОтбора77 = СтруктураДанных.OLE_СпрФирмы.ТекущийЭлемент();
		Иначе
			Сообщить("По строке: "+СтруктураДанных.СтрокаНомер + "Организация " + мОрганизация + " в 1с7 не найдена!");  
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Для каждого СтрУслуги Из МассивУслуги Цикл
			...
			Всего = СтрУслуги.Сумма;
			
			Если Не СтруктураВариантовРасчетаНалога.НДСВСумме Тогда
				Всего = СтрУслуги.Сумма + СтрУслуги.СуммаНДС; 
			КонецЕсли;
			
			СтруктураСтроки.Вставить("Всего", Всего);  
...
Функция ПолучитьСтрокиПоступленияУслугНаСервере(Документ, Отказ)
	...
	Если МассивСтрок.Количество() = 0 Тогда
		// В документе нет услуг, не выгружаем его.
		Отказ = Истина;
	КонецЕсли;
...
Процедура ВыгрузитьПоступлениеУслугПродолжение(СтруктураДанных)
	...
	// Ищем документа в базе-приемнике
	OLE_Документ.НайтиПоНомеру(мНомер, ПолучитьЗначениеРеквизитаНаСервере(СтруктураДанных.Документ, "Дата"));
	
	ПерезаписатьДокумент = Ложь;
	
	Если OLE_Документ.Выбран() = 1 Тогда
	...
	OLE_Документ.Комментарий           = "#Загружено из 1С8 по поступлению №" + ПолучитьЗначениеРеквизитаНаСервере(СтруктураДанных.Документ, "Номер") + " от " + Формат(ПолучитьЗначениеРеквизитаНаСервере(СтруктураДанных.Документ, "Дата"), "ДФ=dd.MM.yyyy");
	
	OLE_Документ.УдалитьСтроки();
	Для каждого СтрМассива Из СтруктураДанных.МассивСтрок Цикл
	...
	Исключение
		Сообщить("Ошибка проведения в 1с7 поступления №" + OLE_Документ.Номер + " от " + Формат(OLE_Документ.ДатаДок, "ДФ=dd.MM.yyyy") + ". Попробуйте провести в 1с7 этот документ вручную!");
	КонецПопытки;
	
	СтруктураДанных.Вставить("НомерДок", OLE_Документ.НомерДок);
	СтруктураДанных.Вставить("ДатаДок",  OLE_Документ.ДатаДок);
	СтруктураДанных.Вставить("ПерезаписатьДокумент", ПерезаписатьДокумент);	
...
Процедура ЗаполнитьПоступлениеУслуг()
	...
	// Результат запроса возвращает все документыпо услугам.
	// Основной признак, что это услуга - это отсутствие склада.
	...
	|		КОНЕЦ
	|	И ПоступлениеТоваровУслуг.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)";
